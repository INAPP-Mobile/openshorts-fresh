...
# Constants
# OUTPUT_DIR / UPLOAD_DIR are env-overridable so a deployment can persist them
...
# ── Health check ────────────────────────────────────────────────────────────
# (Defined before the SPA catch-all so it is registered first.)
@app.get("/health")
async def health():
    return {
        "status": "ok",
...
# into DIST_DIR.  All API calls in the SPA are relative (same origin), so
# a single FastAPI service serves both the API and the UI.
...
if os.path.isdir(DIST_DIR):
    ...
    _index_file = os.path.join(DIST_DIR, "index.html")
    ...
    async def _spa_catchall(path: str):
        # Serve a static file from dist/ if it exists on disk;
        # otherwise return index.html (hash-routed SPA fallback).
        _candidate = os.path.join(DIST_DIR, path)
